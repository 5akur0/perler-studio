...
| `--brand-cta` / `--brand-cta-strong` | `#3D9C8C` / `#389586` | 主按钮**平涂**专用（无渐变） |

> ⚠️ 主按钮 = 白字 + `--brand-cta` **平涂** ≈ **3.3:1**：满足大字 3:1，**不满足正文 4.5:1**——为「更亮观感」的**有意取舍**。不要把这套 cta 色用于小字文本。
...
### 成就材质色
`--achievement-locked #c3cad2` · `--achievement-bronze #b56845` · `--achievement-silver #7d8994` · `--achievement-gold var(--amber)`。成就图案使用严格 `5×5` 网格的纯色像素块，每格整块填充且无内部浅色修边；图案按成就语义专门绘制（如完成=勾选、时长=沙漏、检查=放大镜），不复用图纸剪影。三阶组铜→银→金，两阶组铜→金，四阶组最后回到运行时 `--brand` 表示满级。卡片不再展示材质名、阶段数或阶段表，当前值以无底色的次级文字放在卡片右侧中央，不展示分母；下一目标保留在图案的无障碍名称中。
...
## 2. 表面与层次（★ 手绘墨线素描规范 — 2026-07 定版，DESIGN.md §4 为准）

> 2026-07 语言切换：**退役三层玻璃**，改为**两层「纸片摆在主题背景桌上」**。面板不再高透磨砂，而是纸白实心 + 手绘墨线描边 + 硬贴纸投影。全站直角（`--wobble-*` 已退役，`--r-*` / `--r-pill` 全归零），无歪角、无圆角。DESIGN.md §4「Paper-on-Desk / Ink-Outline / Hard-Sticker-Shadow」为权威，本节与其一致。
...
border-radius: 0;                        /* 全站直角；--wobble-* 已退役，--r-* 全归零 */
box-shadow: var(--sketch-shadow);        /* 3px 3px 0 硬贴纸投影，无模糊；弹窗用 --sketch-shadow-lg */
/* 禁止 backdrop-filter 磨砂；禁止柔性扩散投影 */
```
> 色号 chip 是密度豁免：保留 1px 浅 `--line` 描边（221 色网格全墨边会太吵）。移动端近满幅面板去掉硬投影（外层 `overflow:hidden` 会切影），仅留墨边。唯一的「圆」是真圆形（圆珠色点、滑块 / 开关钮）走 `border-radius:50%`（是形状，不是圆角）。

**L2 · 弹窗**：`--surface` 白底 + 墨边 + **直角** + `--sketch-shadow-lg`。`.remap-modal` 骨架已统一（焦点陷阱 + Esc）；新弹窗一律复用 `.remap-modal` + `modal-controller.js`。
...
**canvas 道具素描语言**（`src/sketch-style.js`）
canvas 绘制的道具与 CSS 面板同语言：平涂、直角、墨线、硬贴纸投影（偏移无模糊）。`src/sketch-style.js` 是 CSS token 在 canvas 侧的镜像常量——改 token 须同步两处。豁免：豆子本体及接触影、玩法反馈光效（台灯辉光/热度/冷却/熔接桥）、作品 sheen、真圆/药丸形状。允许的渐变/shadowBlur 数量由 `scripts/canvas-sketch-regression.mjs` Part 2 固定，变动须更新脚本注释。

---

## 3. 形状 · 间距 · 层叠

- **圆角**：`--r-*` / `--r-pill` 全部为 `0`（2026-07 改版，全站直角，`--wobble-*` 已退役）。token 名称保留（call site 仍引用），但值归零——不再有任何圆角或歪角。唯一的「圆」是真圆形（圆珠色点、滑块/开关钮）走 `border-radius:50%`，是形状而非圆角。
...
- **卡片/入口**：**直角**（`--r-md` = 0），`--surface` 平底 + `--sketch-bw-ctl`(1.5px) 墨边 + `--sketch-shadow-sm` 硬投影；hover 把硬投影加深到 `--sketch-shadow` + `cursor:pointer`，**不位移、不缩放**。
...
- 断点（doc，CSS @media 用字面值）：`sm 420 / phone 620 / mobile 860 / workbench-full 1008 / tablet 1180`。工作台 861–1007 为中央优先紧凑拓扑，≥1008 才启用 22/54/24 三列拓扑。
